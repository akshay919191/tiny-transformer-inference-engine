...
def do(model, tokens):
    """Profile a single forward pass and print the kernel table."""
    model(tokens)  
...
def main():
    ...
    p.add_argument("--profile_fwd", action="store_true",
                   help="run do(): profile a single forward pass and print the kernel table")
    args = p.parse_args()
    ...
    cache = make_cache_fn(model, cfg, 32, device)()
    tok = prefill(model, tokens, cache).argmax(-1, keepdim=True)
